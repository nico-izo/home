...
NeoBundle 'https://code.google.com/p/vim-plugin-viewdoc/', {'type': 'hg'}
...
NeoBundleLazy 'phildawes/racer', {
    ...
    \ 'build': {
    \   'unix': 'cargo clean && cargo build --release',
    \   'mac': 'cargo clean && cargo build --release'
    \   }
    \ }
NeoBundleLazy 'racer-rust/vim-racer', {'autoload': {'filetypes': 'rust', 'filename_patterns': '*.rs'}}
...
" NeoBundle 'doxygen-toolkit', {'type' : 'nosync', 'base' : '~/.vim/bundle'}
NeoBundleLazy 'conque-gdb', {'type' : 'nosync', 'base' : '~/.vim/bundle', 'autoload': {'filetypes': ['cpp']}}

" Цветовая схема
NeoBundle 'Lokaltog/vim-distinguished', 'develop', {'autoload': {'unite_sources': 'colorscheme',}}
...
set noerrorbells visualbell t_vb= " Отключаем моргания экран
...
" au Filetype vim,perl,xml,diff,html,ocaml set nospell " Для вима и перла орфографию отключим
...
let &undodir = $HOME."/.vim/.tmp/undofiles/" " Хранить undo там
let &backupdir = $HOME."/.vim/.tmp/backupfiles/" " Храним бэкапы тут
let &backupskip = &backupskip . "," . &backupdir . '/*' " Не бэкапим содержимое бэкапов
let &directory = $HOME."/.vim/.tmp/swap/" " место под своп-файл

silent! call MakeDirIfNoExists(&undodir)
silent! call MakeDirIfNoExists(&backupdir)
silent! call MakeDirIfNoExists(&directory)

"if isdirectory(s:backupdir . '/.bzr')
"	au BufWritePost * call system("bzr add ".&backupdir.expand(bufname('%')). &backupext )
"	au BufWritePost * call system("bzr commit \".&backupdir." -m Fix")
"endif
" }}}
...
let g:indexer_tagsDirname = $HOME."/.vim/tags/projects"
...
" List {{{
au FileType vim,sh,zsh,bash,html,css,sass,javascript,php,python,ruby,psql setl list
au FileType diff setl list listchars+=trail:\ " Выкинуть trailing пробелы
" }}}
...
" NerdTree {{{
" F12 - открыть/закрыть окно списка файлов
" nmap <silent> <F12> :NERDTreeToggle<cr>
" vmap <silent> <F12> <esc>:NERDTreeToggle<cr>
" imap <silent> <F12> <esc>:NERDTreeToggle<cr>
" autocmd vimenter * NERDTree
map <F12> <plug>NERDTreeTabsToggle<CR>
let g:NERDTreeWinPos='left' " Располагаться NerdTree должно справа
let g:NERDTreeWinSize=35 " Ширина окна NerdTree
...
" Для сниппетов
let g:snips_author="Nicolay Izoderov <nico-izo@ya.ru>"
" Doxygen
let g:DoxygenToolkit_authorName="Nicolay Izoderov <nico-izo@ya.ru>"
" Po.vim
let g:po_translator = "Nicolay Izoderov <nico-izo@ya.ru>"
" }}}
